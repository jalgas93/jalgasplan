...
class AddDataFragment : Fragment() {
    ...
    private fun initialization() {
        ...
        var sideName = mBinding.idName.text.toString()
        var Name = mBinding.name.text.toString()
        var AddressName = mBinding.addressName.text.toString()
        if (sideName.isNotEmpty() && Name.isNotEmpty() && AddressName.isNotEmpty()) {
            mViewModel.insert(Model(name = Name, id_name = sideName, address_name = AddressName)) {
                findNavController().navigate(R.id.action_addDataFragment_to_generalFragment)
            }
